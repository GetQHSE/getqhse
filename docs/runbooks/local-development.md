...
3. Keep `DATABASE_URL` on the runtime `qhse_app` role and `MIGRATION_DATABASE_URL` on the
   schema-owning `qhse_migrator` role. Apply committed migrations with
   `pnpm exec dotenv -e .env -- pnpm db:deploy`, then seed with
...
Prisma migration commands prefer `MIGRATION_DATABASE_URL`; application code continues to use
`DATABASE_URL`. Prisma uses the disposable `qhse_shadow` database initialized by Compose for drift
detection.
