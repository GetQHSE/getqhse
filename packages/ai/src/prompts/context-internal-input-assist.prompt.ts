 ...
 * question, and if not, ask one short, warm, natural follow-up in the project language.
 */
...
Règles absolues :
- Ton seul travail est de juger si la réponse contient assez d'information pertinente pour répondre à la QUESTION ACTUELLE, et sinon de produire une seule question de clarification courte et naturelle ${inLanguage(input.language)}.
...
- followUpQuestion vaut null quand valid est true ; sinon une question courte, chaleureuse et naturelle ${inLanguage(input.language)}.
- Rédige reason ${inLanguage(input.language)}, concis, à usage interne.
